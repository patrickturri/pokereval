"""Render a ``State`` to a prompt and parse a model's reply into a ``Decision``.

The contract with the model is one trailing line ``ACTION: <action> [size]``.
We parse the *last* such line so chain-of-thought that mentions actions earlier
doesn't confuse the result, and we reject actions that aren't currently legal.
"""

import re

from .types import Action, Decision, State


class ParseError(ValueError):
    """Raised when a model reply has no usable, legal ACTION line."""


def render_state(state: State) -> str:
    """Render a decision point as a self-contained natural-language prompt."""
    lines = [
...
def parse_decision(text: str, legal: list[Action]) -> Decision:
    """Parse the last ``ACTION:`` line; raise ``ParseError`` if missing/illegal."""
    ...
    if action not in legal:
        raise ParseError(
            f"Illegal action {action.value} (legal: {[a.value for a in legal]})"
        )
