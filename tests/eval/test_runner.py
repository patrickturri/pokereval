from pokereval.eval.runner import run_eval
...
def test_run_eval_grades_a_valid_decision():
    ss = _spotset()
    results = run_eval(
        FakeClient("m", "ACTION: raise"), ss, VerifiableGrader(ss.nash_probs)
    )
...
def test_run_eval_records_parse_errors():
    ss = _spotset()
    results = run_eval(
        FakeClient("m", "no action"), ss, VerifiableGrader(ss.nash_probs)
    )
